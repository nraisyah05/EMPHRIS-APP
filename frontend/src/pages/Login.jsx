...
const Login = () => {
    ...
    // Fungsi untuk mendapatkan peran pengguna dari tabel public.users
    const getUserRole = async (userId) => {
        const { data, error } = await supabase
            .from('users') // Menggunakan tabel 'users' yang kita buat
            .select('role')
            .eq('user_id', userId)
            .single();
    ...
    const redirectByRole = (role) => {
        if (role === 'hr') {
            ...
            // Opsi fallback jika role tidak dikenal
    ...
    const handleEmailLogin = async (e) => {
        ...
        try {
            // 1. Lakukan autentikasi email dan password
            ...
            // 2. Jika autentikasi berhasil, ambil user ID
            const userId = data.user.id;
            
            // 3. Ambil peran pengguna dari tabel 'public.users'
            const userRole = await getUserRole(userId);

            if (userRole) {
                // 4. Redirect berdasarkan peran yang didapat
                redirectByRole(userRole);
            } else {
                setErrorMessage('Akun tidak memiliki peran yang valid di sistem. Silakan hubungi admin.');
            }
        } catch (error) {
    ...
    const handleGoogleLogin = async () => {
        ...
        const redirectUrl = isDevelopment
            ? `${window.location.origin}/handle-redirect`
        ...
        const { data: authListener } = supabase.auth.onAuthStateChange(
            async (event, session) => {
                if (event === 'SIGNED_IN' && session) {
                    ...
                    const userRole = await getUserRole(userId); // Menggunakan fungsi getUserRole

                    if (!userRole) {
                        setErrorMessage('Akun Anda tidak memiliki peran yang valid di sistem. Silakan hubungi admin.');
                ...
                <div className="p-4 text-center">
                    ...
                    <form onSubmit={handleEmailLogin}>
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full py-2 px-4 rounded-2 bg-[#3aba42] text-white font-medium cursor-pointer transition-all duration-200 hover:bg-[#2d8d32] active:scale-98 disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                        >
                    ...
                    <button
                        ...
                        disabled={loading}
                        className="flex items-center justify-center gap-2 w-full py-2 px-4 border border-gray-300 rounded-2 bg-white text-gray-700 font-medium cursor-pointer transition-all duration-200 hover:bg-gray-100 active:scale-98 disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                    >
                        <img
